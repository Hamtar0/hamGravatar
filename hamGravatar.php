	...
	**/
	...
	public function AdminUsersFoot() {?>
		...
		$(document).ready(function(){
			...
			$('<th></th>').addClass('title gravatar').text('Gravatar').insertBefore('#users-table th:eq(0)');
			...
			$.each(table_profils, function(i, item) {
				...
				$('<td></td>').html('<a title="Envoyer un email" href="mailto:'+item.email+'" target="_top"><img src="'+gravatarimg+'" /></a>').insertBefore('#users-table tbody tr:eq('+(i-1)+') td:eq(0)');

			});

			$('<td></td>').insertBefore('#users-table tbody tr:last td:eq(0)');
	...
	public function AdminUserTop() {?>
		...
		$(document).ready(function(){
			...
			$('fieldset').css('background', 'url("' + gravatarimg + '") no-repeat top 20px right 20px ');
